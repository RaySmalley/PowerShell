...
if ($deviceType -eq 1) { Start-Process -FilePath "$output" -ArgumentList "/key=$($args[0]) /group=-45078939940020 /silent" -Wait }
if ($deviceType -eq 2 -or $deviceType -eq 3) { Start-Process -FilePath "$output" -ArgumentList "/key=$($args[0]) /group=2928509553360002652 /silent" -Wait }
if (Get-Process WRSA -ErrorAction SilentlyContinue) {
